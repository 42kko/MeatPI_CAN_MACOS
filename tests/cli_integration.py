import os, subprocess, tempfile, socket, concurrent.futures, time, json, shutil, sys
binary=os.path.abspath(sys.argv[1])
runtime=tempfile.mkdtemp(prefix='meatcan-qa-',dir='/private/tmp');os.chmod(runtime,0o700)
env={**os.environ,'MEATCAN_RUNTIME_DIR':runtime}; results=[]
def run(*args):return subprocess.run([binary,*args],env=env,text=True,capture_output=True,timeout=8)
def check(name,value):
 results.append((name,bool(value)))
 if not value:raise AssertionError(name)
def raw(line):
 s=socket.socket(socket.AF_UNIX);s.settimeout(4);s.connect(runtime+'/daemon.sock');s.sendall(line);return s
try:
 ...
 for s in subscribers:check('dump handshake',s.recv(20)==b'OK\n')
 check('send quiet', (lambda r:r.returncode==0 and not r.stdout and not r.stderr)(run('send','123#01020304')))
 for s in subscribers:check('dump frame',b'can0 123#01020304' in s.recv(4096));s.close()
 ...
 s=raw(b'UNKNOWN\n');check('unknown IPC command',s.recv(200).startswith(b'ERR '));s.close()
 s=raw(b'x'*300+b'\n');check('oversized IPC command',s.recv(200).startswith(b'ERR '));s.close()
 check('down',run('down').returncode==0);time.sleep(.1)
 ...
 for mode, expected in [('drop','TX echo timeout'),('mismatch','mismatched TX echo')]:
  ...
  check(mode+' down',run('down').returncode==0);time.sleep(.1)
 env['MEATCAN_MOCK_ECHO']='delay'
 check('delay up',run('up','--mock').returncode==0)
 started=time.monotonic(); delayed=run('send','123#0102')
 check('waits actual echo',delayed.returncode==0 and time.monotonic()-started>=.09)
 orphan=raw(b'SEND 123#0102\n');time.sleep(.025);orphan.close()
 # Keep new requests active while the abandoned request's delayed echo arrives.
 for i in range(20):
